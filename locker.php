...
<body>
	...
	if (isset($_POST["submit"])) {
		...
		echo '<script>alert("Locker Created");</script>';
	}
	...
	<main>
		...
		<a href="createlocker.php"><button type="submit" class="btn btn-primary" value="Create" name="submit">Create New Locker</button></a> <br><br>
		<form id="create-locker-form" style="display:visible;" action="locker.php" method="post">
			...
			$sql = "SELECT tbl_locker.id, tbl_locker.name, count(movie_id) as movie_count FROM `tbl_locker` left join tbl_lockermovies on id = locker_id WHERE tbl_locker.`user id` = " . $_SESSION["userid"] . " Group by tbl_locker.id;";
			$result = mysqli_query($conn, $sql);
